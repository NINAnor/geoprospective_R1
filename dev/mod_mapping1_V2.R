    ...
    update_final_map <- function() {
      ...
      output$rating<-renderUI( 
        tagList(
          ...
          div(
            id = "blog_input_container",  # Add a unique ID to the container
            textInput(ns("blog"), label = "") %>%
              shinyInput_label_embed(
                icon("info") %>%
                  bs_embed_tooltip(
                    title = "Make it clear to other people why your areas are important. Please do this in a short blog-like statement.",
                    placement = "right"
                  )
              )
          ),
